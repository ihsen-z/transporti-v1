    ...
    const renderStars = (rating: number, size: 'sm' | 'xs' = 'sm') => (
        <div className="flex items-center gap-0.5">
            {[1, 2, 3, 4, 5].map(star => (
                <Star
                    key={star}
                    className={`${size === 'sm' ? 'w-4 h-4' : 'w-3 h-3'} ${star <= rating
    ...
    return (
        <div className="bg-white rounded-xl border border-neutral-200 p-5">
                    ...
                    {Object.entries(r.aspects).map(([key, value]) => {
                        ...
                        return (
                            <div key={key} className="flex items-center gap-1.5">
                                <span className="text-xs text-neutral-500">{ASPECT_LABELS[key] || key}</span>
            ...
            {r.would_recommend !== undefined && (
                <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${r.would_recommend
                        ? 'bg-emerald-50 text-emerald-700'
                        : 'bg-neutral-100 text-neutral-500'
                    }`}>
                    <ThumbsUp className={`w-3.5 h-3.5 ${r.would_recommend ? '' : 'rotate-180'}`} />
