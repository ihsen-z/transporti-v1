...
export default function JobActions({ status, onStatusChange, jobId }: JobActionsProps) {
    ...
    // Generic action handler with loading simulation
    const handleAction = useCallback(async (
        actionName: string,
        newStatus: JobStatus | null,
        successMessage: string,
        delay: number = 800 + Math.random() * 400
    ) => {
        setIsLoading(true);
    ...
    }, [onStatusChange, showFeedback]);

    // Action handlers by status
    const handleAccept = () => handleAction(
        'accept',
        'ACCEPTED',
        'Livraison acceptée ! Un transporteur vous sera assigné.'
    );

    const handleRefuse = () => handleAction(
        'refuse',
        'CANCELLED',
        'Livraison refusée.'
    );

    const handleStartTransport = () => handleAction(
        'start',
        'IN_PROGRESS',
    ...
    const handleConfirmDelivery = () => handleAction(
        'confirm',
        'COMPLETED',
    ...
    const getButtonStyles = (variant: ActionButton['variant'], isActionLoading: boolean) => {
        const base = 'w-full flex items-center justify-center gap-2 font-semibold py-3 px-4 rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed';
            ...
            <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
                ...
                <div className="flex items-center gap-3 p-4 bg-neutral-100 rounded-xl">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-neutral-500">Ce transport a été annulé</p>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
            ...
            {status === 'COMPLETED' && (
                <div className="flex items-center gap-3 p-4 bg-accent-50 rounded-xl mb-4 border border-accent-200">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-accent-600">Le colis a été livré avec succès</p>
                    </div>
                </div>
            )}
            ...
            {/* Feedback Message */}
            {feedback && (
                <div className={`flex items-center gap-3 p-3 rounded-lg border mb-4 animate-pulse ${getFeedbackStyles(feedback.type)}`}>
            ...
            <div className="space-y-3">
                {actions.map((action, index) => {
                    const isActionLoading = isLoading && loadingAction === action.label.toLowerCase().replace(/\s/g, '_');
                    ...
                    return (
                        <button
                            ...
                            disabled={isLoading}
                            className={getButtonStyles(action.variant, isLoading && loadingAction !== null)}
                        >
                            {isActionLoading ? (
            ...
            {status !== 'COMPLETED' && actions.length > 0 && (
                <p className="text-xs text-neutral-500 mt-4 text-center">
                    Transport #{jobId} • Les actions sont simulées localement
                </p>
            )}
        </div>
    );
