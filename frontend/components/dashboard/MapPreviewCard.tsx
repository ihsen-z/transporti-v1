...
export default function MapPreviewCard() {
    ...
    const activeJob = mockRecentJobs.find(j => j.status === 'IN_PROGRESS' || j.status === 'ACCEPTED') || mockRecentJobs[0];
        ...
        <div className="bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden">
            <div className="p-4 border-b border-neutral-200">
                <div className="flex items-center justify-between">
                    <h3 className="text-lg font-bold text-neutral-900">Transport actif</h3>
                    <Map className="w-5 h-5 text-primary-600" />
                </div>
                <p className="text-sm text-neutral-600 mt-1 line-clamp-1">{activeJob.title}</p>
            </div>
            <RouteMap
                pickup={{
