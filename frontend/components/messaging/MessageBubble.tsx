...
export function MessageBubble({ content, timestamp, isSender, senderName, isRead }: MessageBubbleProps) {
    const timeLabel = (() => {
        try {
    ...
    return (
        <div className={`flex ${isSender ? 'justify-end' : 'justify-start'} mb-3`}>
            <div className={`max-w-[75%] ${isSender ? 'order-2' : ''}`}>
                {/* Sender Name */}
                {!isSender && (
                    <p className="text-xs font-medium text-neutral-500 mb-1 ml-1">{senderName}</p>
                )}
                ...
                {/* Meta */}
                <div className={`flex items-center gap-1.5 mt-1 ${isSender ? 'justify-end' : 'justify-start'} px-1`}>
                    <span className="text-[10px] text-neutral-400">{timeLabel}</span>
                    {isSender && isRead !== undefined && (
                        <span className={`text-[10px] ${isRead ? 'text-blue-500' : 'text-neutral-400'}`}>
                            {isRead ? '✓✓' : '✓'}
                        </span>
                    )}
                </div>
