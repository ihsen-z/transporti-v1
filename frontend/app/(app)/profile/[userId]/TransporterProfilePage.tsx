...
/* -------------------------------------------------------------------------- */
/*  Transporter Profile Page — extracted from page.tsx (god-file split)      */
/* -------------------------------------------------------------------------- */

export default function TransporterProfilePage({ userId }: { userId: string | null }) {
  ...
  const handleAvatarCropped = async (blob: Blob) => {
    ...
    try {
      ...
      setSaveMsg({
        type: "err",
        text: getErrorMessage(err) || t_i18n.profile.avatarError,
      });
    } finally {
  ...
  const handleVehicleCropped = async (blob: Blob) => {
    ...
    try {
      ...
      setSaveMsg({
        type: "err",
        text: getErrorMessage(err) || t_i18n.profile.vehiclePhotoError,
      });
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const errBody = (e instanceof ApiError ? e.body : undefined) as
  ...
  const fetchProfile = async () => {
    ...
    try {
      const [profileData, reviewsData] = await Promise.all([
        ...
        apiClient.get<ReviewData[] | { results?: ReviewData[] }>(
      ...
      setError(getErrorMessage(e) || t_i18n.profile.profileNotFound);
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className="relative flex-shrink-0">
                  ...
                  >
                    {(editing ? editAvatarUrl : t.avatar_url) ? (
                      <Image
                        src={getMediaUrl((editing ? editAvatarUrl : t.avatar_url) || "")}
                        alt={fullName}
                        width={80}
                        height={80}
            ...
            <div className="p-5">
              {editing ? (
                ...
                <p className="text-sm text-neutral-600 leading-relaxed">
                  {t.bio || <span className="italic text-neutral-400">{t_i18n.profile.noInfo}</span>}
                </p>
              )}
            </div>
          </div>
