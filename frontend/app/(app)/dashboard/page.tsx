...
const MOCK_RECENT_JOBS: RecentJob[] = [
    { id: 1, job_type: 'TRANSPORT', status: 'PUBLISHED', pickup_address: 'Tunis — La Marsa', dropoff_address: 'Sousse Centre', scheduled_time: '2026-02-12T10:00:00Z', offer_count: 3 },
    { id: 2, job_type: 'MOVING', status: 'IN_PROGRESS', pickup_address: 'Ariana — Ennasr', dropoff_address: 'La Soukra', scheduled_time: '2026-02-11T08:00:00Z', offer_count: 1 },
    { id: 3, job_type: 'TRANSPORT', status: 'COMPLETED', pickup_address: 'Sfax Ville', dropoff_address: 'Gabès', scheduled_time: '2026-02-09T14:00:00Z', offer_count: 5 },
];
...
function StatusBadge({ status }: { status: string }) {
    ...
    return (
        <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${styles[status] || 'bg-neutral-100 text-neutral-600'}`}>
...
function ClientDashboard({ stats, recentJobs }: { stats: ClientStats; recentJobs: RecentJob[] }) {
    return (
        <>
            {/* CTA Hero */}
                            ...
                            <div className="flex items-center gap-4 min-w-0">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-xs text-neutral-500">
                                        {new Date(job.scheduled_time).toLocaleDateString('fr-TN', { day: 'numeric', month: 'short', year: 'numeric' })}
                                        {job.offer_count !== undefined && ` · ${job.offer_count} offre${job.offer_count > 1 ? 's' : ''}`}
                                    </p>
                                </div>
                            </div>
...
function TransporterDashboard({ stats, recentJobs }: { stats: TransporterStats; recentJobs: RecentJob[] }) {
    return (
        <>
            {/* Verification Banner */}
            {stats.verification_status !== 'VERIFIED' && (
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 mb-8 flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <Link href="/verification" className="inline-flex items-center gap-1 text-sm font-semibold text-amber-800 mt-3 hover:text-amber-900">
...
export default function DashboardPage() {
    ...
    return (
        <div className="p-6 lg:p-8 max-w-7xl mx-auto">
            ...
            {isClient && <ClientDashboard stats={clientStats} recentJobs={recentJobs} />}
            {isTransporter && <TransporterDashboard stats={transporterStats} recentJobs={recentJobs} />}

            {/* Fallback */}
            {!isClient && !isTransporter && (
                <div className="bg-white rounded-2xl border border-neutral-100 p-12 text-center">
                    <BarChart3 className="w-12 h-12 text-neutral-300 mx-auto mb-4" />
                    <h2 className="text-lg font-semibold text-neutral-900">Bienvenue sur Transporti</h2>
                    <p className="text-neutral-500 mt-1">Sélectionnez votre rôle pour voir votre tableau de bord.</p>
                </div>
            )}
        </div>
    );
