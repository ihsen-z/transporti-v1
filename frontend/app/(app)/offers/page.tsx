...
import { FileText, Clock, CheckCircle, XCircle, AlertCircle, TrendingUp, DollarSign } from 'lucide-react';
...
export default function MyOffersPage() {
    ...
    const handleWithdraw = (offerId: number) => {
        if (confirm('Êtes-vous sûr de vouloir retirer cette offre ?')) {
            setOffers(prev =>
                prev.map(o => o.id === offerId ? { ...o, status: 'WITHDRAWN' as any } : o)
            );
        }
    };
        ...
        <div className="p-6 lg:p-8 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
                ...
                <div className="bg-white rounded-xl border border-neutral-200 p-4">
                    ...
                    <p className="text-2xl font-bold text-blue-600">{potentialEarnings.toFixed(0)} TND</p>
                </div>
            </div>
            ...
            <div className="flex gap-1 bg-neutral-100 rounded-xl p-1 mb-6 overflow-x-auto">
                {TABS.map(tab => {
                    const count = tab.id === 'ALL' ? offers.length : offers.filter(o => o.status === tab.id).length;
                    return (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${activeTab === tab.id
                        ...
                        >
                            ...
                            {tab.label}
                            <span className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? 'bg-blue-50 text-blue-600' : 'bg-neutral-200 text-neutral-500'
                                }`}>
                                {count}
                            </span>
                        </button>
                ...
                {filtered.length === 0 ? (
                    <div className="text-center py-16 bg-white rounded-xl border border-neutral-200">
                        ...
                        <p className="text-neutral-500 font-medium">
                            Aucune offre {activeTab !== 'ALL' ? `${TABS.find(t => t.id === activeTab)?.label.toLowerCase()}` : ''} pour le moment.
                        </p>
