...
export default function LoginPage() {
  ...
  const handleDemoLogin = async () => {
    ...
    showToast(
      "success",
      `Connexion réussie en tant que ${roleLabels[selectedRole]}`,
    );
    const redirectTo =
      searchParams.get("redirect") || getDefaultRedirect(selectedRole);
    router.push(redirectTo);
  };
              ...
              {roles.map(({ value, icon: Icon, description }) => (
                ...
                >
                  ...
                  <div className="text-left flex-1">
                    <p
                      className={`font-semibold ${selectedRole === value ? "text-brand-600" : "text-neutral-800"}`}
                    >
                      {roleLabels[value]}
                    </p>
